...
mrvaDir='/tmp/nrtMRVAtest'
#makebicDir='/tmp/makebic'
makebicDir='/tmp/makebictest'
...
scanLatency = 4 # [days] defines begin date.
...
#   /home/tmchin/ice/saf2bip.m
# or
#   /home/tmchin/ice/p01/saf2bip.m
...
sensors=( # sensor, binRootDir, region, La, Lb, dayrange, stablat:
  ...
  ('MODIST','/nas2/bic/MODIST','Global',2,12, 2, 2),
...
for year in span(year0,year2):
  ...
  for day in span(d0,d2):
    ...
    fice = open(icefile,'w')
    fice.write("path('/home/tmchin/ice',path);")
    fice.write("year=%d; day=%d;"%(year,day))
    fice.write("[icesstfile,landicefile]=saf2bip(year,day);")
    ...
    fice.write("rmpath('/home/tmchin/ice');")

    fice.write("path('/home/tmchin/ice/p01',path);")
    fice.write("year=%d; day=%d;"%(year,day))
    fice.write("[icesstfile,landicefile]=saf2bip(year,day);")
    fice.write("if length(landicefile),")
    fice.write("eval(sprintf('! gzip -f %s &',landicefile)); end;")
