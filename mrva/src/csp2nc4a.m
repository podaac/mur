...
%      landmaskfile='tmp.gds';
      ...
      icemap=fortread(f,'integer*1',[ii,jj]);
      fclose(f);
    else,
    ...
    [msst,mlon,mlat]=fortread(f,'integer*2',[ii,jj],'real*4',ii,'real*4',jj);
    msst=int16(msst); mlon=single(mlon); mlat=single(mlat);
    fclose(f);
...
%    ! rm -f tmp.gds;
    ...
    if 1,  % turn them to simple "land" flag:
      inx=find(mask(:)==3); if length(inx), mask(inx)=int8(2); end;
      inx=find(mask(:)==7); if length(inx), mask(inx)=int8(2); end;
      inx=find(mask(:)==11); if length(inx), mask(inx)=int8(2); end;
      inx=find(mask(:)==15); if length(inx), mask(inx)=int8(2); end;
    end;
