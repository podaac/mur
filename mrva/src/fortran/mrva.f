...
!            if(L.ge.11) epsln=1.0e-6
            ...
            print*,'Starting PCG solver (maxIter=',mx3*my3,')'
            call flush(6)
            call spmPCG(csp,mx3*my3,epsln,1.0e3)
            print*,'PCG solver completed'
            call flush(6)
          end if

        ! data residual:
          print*,'Computing data residuals...'
          call flush(6)
          do m=1,nbipfile
            if(L.ge.d(m)%La .and. L.le.d(m)%Lb) then
...
!            end if
          ...
          print*,'Data residuals completed'
          call flush(6)


        ! accumulate:
          print*,'Accumulating coefficients...'
          call flush(6)
          csp(:)=dsp(:)+csp(:)
          deallocate(dsp)

        ! write coefficient field:
          print*,'Writing coefficient file...'
          call flush(6)
        ...
        ! write uncertainty coefficient field:
          print*,'Writing uncertainty coefficient file...'
          call flush(6)
       ...
       ! write map file:
          print*,'Writing globe map...'
          call flush(6)
          call writeGlobeMap(csp,L)
          print*,'Globe map completed'
          call flush(6)

          print*,'Level L=',L,' completed'
          print*,''
          call flush(6)
