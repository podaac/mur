...
!
      ...
      if(ios.ne.0) then
        print*,'CBS Error opening "cbsdata.out"; abort'
        stop
      end if
      read(8) mx,my,mz,nv
...
!
      ...
      do n=1,nv
        ...
        if(grid) then
            ...
            do i=1,nx
              k=nx*(j-1)+i
              call spmpoint(1,1,csp,xo(i),yo(j),lx,ly,1.,1.,zo(k,n))
            enddo
            enddo
        else
            do k=1,nd
              call spmpoint(1,1,csp,xo(k),yo(k),lx,ly,1.,1.,zo(k,n))
            enddo
        endif
